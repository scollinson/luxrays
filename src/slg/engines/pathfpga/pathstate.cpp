...
PathFPGAState::PathFPGAState(PathFPGARenderThread *renderThread,
		Film *film, luxrays::RandomGenerator *rndGen) : FPGARenderState(renderThread, film, rndGen), sampleResults(1) {
	PathFPGARenderEngine *renderEngine = (PathFPGARenderEngine *)renderThread->renderEngine;
...
void PathFPGAState::Init(const PathFPGARenderThread *thread) {
	PathFPGARenderEngine *renderEngine = (PathFPGARenderEngine *)thread->renderEngine;
...
void PathFPGAState::DirectHitInfiniteLight(const Scene *scene, const Vector &eyeDir) {
	// Infinite light
	float directPdfW;
	if (scene->envLight) {
...
void PathFPGAState::DirectHitFiniteLight(const Scene *scene, const float distance, const BSDF &bsdf) {
	float directPdfA;
...
void PathFPGAState::DirectLightSampling(const PathFPGARenderThread *renderThread,
...
bool PathFPGAState::FinalizeRay(const PathFPGARenderThread *renderThread,
...
void PathFPGAState::SplatSample(const PathFPGARenderThread *renderThread) {
	sampler->NextSample(sampleResults);

	Init(renderThread);
}

void PathFPGAState::GenerateRays(FPGARenderThread *renderThread) {
	PathFPGARenderThread *thread = (PathFPGARenderThread *)renderThread;
...
double PathFPGAState::CollectResults(FPGARenderThread *renderThread) {
	PathFPGARenderThread *thread = (PathFPGARenderThread *)renderThread;
	PathFPGARenderEngine *renderEngine = (PathFPGARenderEngine *)thread->renderEngine;
	Scene *scene = renderEngine->renderConfig->scene;
